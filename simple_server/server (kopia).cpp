...
void *ThreadBehavior(void *t_data)
{
    ...
    while(czy_polaczony)
    {
        ...
        printf("Oczekiwanie na login\n");
        write(th_data->connection_socket_descriptor,"log",BUFF_SIZE);
        int read_result = read(th_data->connection_socket_descriptor, buffor, BUFF_SIZE);
        if (read_result>0) {
            write(th_data->descriptor_array[th_data->connection_socket_descriptor], buffor, BUFF_SIZE);
        } else printf("Blad");

        if (th_data->connection_socket_descriptor > th_data->descriptor_array[th_data->connection_socket_descriptor]) {
